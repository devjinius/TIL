...
이 경우 잘 작동하지만 좋은 방식은 아니다. 그 이유는 foo()라는 이름의 함수를 선언해야 한다. 즉 글로벌 스코프의 foo라는 Identifier를 사용해야만 한다. 또한 그 함술르 foo()로 호출해야만 실제 코드를 실행시킬 수 있다. 이게 무슨소린가 싶겠지만 한 번만 사용하는 함수라면 굳이 선언하고 실행하는 과정을 없애도 되지 않을까?
...
이렇게 하면 foo()라는 함수는 글로벌 스코프의 확인자를 건드리지 않으면서 (즉 스코프에 영향을 주지 않으면서) 바로 실행된 후 날아가버린다. '()'이렇게 감싸게 되면 function이라는 키워드는 선언문이 아니라 표현식으로 취급된다. 

자 조금 더 자세하게 비교해보자. 첫째 코드에서 함수 이름 foo는 글로벌 스코프에 묶이게 되고 foo()라는 호출명령을 통해 호출됐다. 두번째 코드에서는 함수 자신의 내부 스코프에 묶이게 된다. 즉 (function foo(){})에서 Identifier foo는 오직 {}로 감싸진 구현부에서만 찾을 수 있고 바깥에서는 찾을 수 없다. 함수 이름 foo를 자기 자신 내부에 숨겨버린 것이다. 자신을 실행시킨 후 사라져 버린다.

#### 익명함수

위의 두번째 코드가 익명함수 표현식이다. 익명함수는 표현식에서 사용되기 때문에 그렇게 이야기 한다. 
...
이런식으로 말이다. 익명함수 표현식은 매우 간편하기 때문에 자주 사용되는데 여기에 몇 가지 유의사항을 알아두자.

1. 익명함수는 스택 추적 시 표시할 이름이 없어 디버깅이 더 어려울 수 있다.	
2. 이름없이 함수 스스로 재귀 호출을 하려면 arguments.callee참조가 필요한데 이건 없어질 예정이다.
3. 이름은 보통 readable, understandable에 도움이 되는데 익명함수는 이 이름을 생략한다. 이름을 써주는게 어떨까?

익명이든 기명이든 사실 중요하지 않다. 인라인 함수 표현식을 사용한다는데에 많은 이점이 있다. 따라서 함수 표현식에 이름을 적어준다면 부작용을 많이 없애고 단점을 해결할 수 있게 된다. 따라서 익명함수표현식을 기명함수표현식으로 사용하는 것이 더 바람직해 보인다.
...
스코프에 대한 전반적인 것을 알았으니 이제 전역스코프에서 변수를 선언하면 모든 곳에서 사용된다는 것을 알았다. 그런데 전역 객체가 있는 환경에서 스크립트가 실행될 경우 전역 스코프의 변수는 전역객체에도 역시 생겨난다. 예를들어 브라우저에서 실행할경우 window객체의 자원으로 들어간다. 

그런데 id속성으로 dom요소를 만들어도 전역변수가 생성된다.
...
console.log(foo); // HTML ELEMENTS
```

 이렇게 실행되어버린다.  따라서 전역변수의 사용은 가급적 자제하는 편이 좋다